...
for(var.n in nodenr:nodenr){
  ...
  parms <- data.frame(size = size.val)
  ...
  out.list <- mcmapply(nn.tenfold.tune,train=model.input.list.train,test=model.input.list.test,
                       fml = model.string,size=size.vec,proc=rep(list(proc),length(size.vec)),mc.cores=NC, SIMPLIFY = FALSE)
